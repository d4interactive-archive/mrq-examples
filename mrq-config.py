...
"""
#MongoDB settings
MONGODB_JOBS = "mongodb://127.0.0.1/mrq" # MongoDB URI for the jobs, scheduled_jobs & workers database.Defaults to mon$
...
REDIS_MAX_CONNECTIONS = 2500  #Redis max connection pool size.Defaults to 1000.
...
DEFAULT_JOB_RESULT_TTL = 600 #Seconds the results are kept in MongoDB when status is success.Defaults to 604800 seconds which is 7 days.
DEFAULT_JOB_ABORT_TTL = 600 #Seconds the tasks are kept in MongoDB when status is abort.Defaults to 86400 seconds which is 1 day.
DEFAULT_JOB_CANCEL_TTL = 600 #Seconds the tasks are kept in MongoDB when status is cancelDefaults to 86400 seconds which is 1 day.
...
MAX_MEMORY = 300 # Max memory (in Mb) after which the process will be shut down. Use with PROCESS = [1-N] to have supervisord automatically respawn the worker when this happens.Defaults to 1
...
DASHBOARD_QUEUE = "index_batch" #Default queue for dashboard actions.
